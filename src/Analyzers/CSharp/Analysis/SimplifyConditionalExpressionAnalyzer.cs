    ...
    public class SimplifyConditionalExpressionAnalyzer : BaseDiagnosticAnalyzer
    {
        ...
        private static void AnalyzeConditionalExpression(SyntaxNodeAnalysisContext context)
        {
            ...
            else if (trueKind == SyntaxKind.FalseLiteralExpression)
            {
                ...
                /// a ? false : b >>> !a && b
                else if (AnalyzerOptionDescriptors.SimplifyConditionalExpressionWhenItIncludesNegationOfCondition.IsEnabled(context)
                    && context.SemanticModel.GetTypeInfo(info.WhenFalse, context.CancellationToken).ConvertedType?.SpecialType == SpecialType.System_Boolean)
                {
                    ReportDiagnostic();
                }
            ...
            {
                // a ? b : true >>> !a || b
                if (AnalyzerOptionDescriptors.SimplifyConditionalExpressionWhenItIncludesNegationOfCondition.IsEnabled(context)
